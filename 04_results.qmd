...
pt <- bind_rows(
  ...
  sann_results$params |> 
    ungroup() |> 
    summarise(across(radius:error, ~mean(.))) |> 
    mutate(run = "MEAN")
) |> 
  rename(
    `$\\rho$`     = minpts,
...
low $\rho$, and Run 4 a low $\tau$. The mean error of `r pt[["Error"]][5]` implies
the algorithm predicts `r round(100-pt[["Error"]][5]/25*100, 2)` percent of
points correctly across the 25 user-days.
...
Researchers with unlabeled location-based services data may use the DBSCAN-TE 
algorithm directly using the parameters identified in this research, but should
exercise caution. The most sensitive
parameter in other data sets is likely to be $\rho$, the minimum points to constitute
a cluster: with less temporally dense data, fewer points will accumulate and 
the 200 point threshold may not be a viable option. Understanding the relationship
between data density and the values of these parameters is important future research.
...
Further research should also explore how many labeled user-days are sufficient to 
identify stable parameters in the DBSCAN-TE algorithm versus train an AI to perform
